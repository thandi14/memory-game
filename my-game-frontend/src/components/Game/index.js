...
const findConnectedBubbles = (bubbles, startIndex, color) => {
    ...
    while (queue.length > 0) {
        ...
        // Get neighbors (Top, Bottom, Left, Right)
        const neighbors = [
            index - 13, // Top
            index + 13, // Bottom
            index - 1,  // Left
...
const removeAndApplyGravity = (bubbles, indexesToRemove) => {
    ...
    Object.keys(columns).forEach(col => {
        ...
        colBubbles.forEach((bubble, row) => {
            sortedBubbles.push({ ...bubble, y: row * 30 });
        });
    });
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setShooterBubble(prev => {
                ...
                if (newX < 0 || newX > boardRef.current.offsetWidth - bubbleSize) {
                    bounceX = -velocity.x;
                }
                if (newY < 0 || newY > boardRef.current.offsetHeight - bubbleSize) {
                    bounceY = -velocity.y;
                }
                ...
                if (collisionIndex >= 0) {
                    ...
                    if (connectedIndexes.length >= 2) {
                        ...
                        if (newBubbles.length === 0) {
                            alert('You won! All bubbles have disappeared.');
                        }
                    } else {
                        // Bubbles do not match, place it where it hit
                        const newBubble = { color: shooterBubble.color, x: newX, y: newY };
                        setBubbles([...bubbles, newBubble]);
                    }

                    const getRandomColor = () => ['red', 'blue', 'green', 'purple'][Math.floor(Math.random() * 4)];
                    setShooterBubble({
